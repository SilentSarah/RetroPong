...
class Tournament:
	...
	def check_user(self, user_id):
		if (not (str(user_id) in GameConsumer.user_matches)):
			GameConsumer.user_matches[str(user_id)] = {
	...
	def check_start(self):
		...
		if (self.full()):
			for i in range(0, len(self.players), 2):
				...
				GameConsumer.games[new_game.id()] = new_game
				GameConsumer.user_matches[str(self.players[i])][5] = new_game.id()
				GameConsumer.user_matches[str(self.players[i + 1])][5] = new_game.id()
				self.started = True
