...
function MessageButton(data_user) {
    ...
    target_user.forEach(user => {
        user.addEventListener('click',async () => {
            ...
            const ids=data?.messages?.filter(it => it.isRead === false && it.sender!==current_user).map(it=>it.id)
            console.log(ids,data,`http://localhost:8000/chat/${user_id}/${target_id}/`)
            MessageIsRead(ids)
            scroll_chat.click()
            elemet = user
            Send();
...
async function AcceptDeclineBlockUnblock(id, status)
{
    ...
    await  Fetching(`http://localhost:8000/friends/${current_user}/${id}/${status}/`,'GET',"")
    fetch_info_user()
}
